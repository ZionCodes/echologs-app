  ...
  <div style="max-width:800px;margin:0 auto">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:40px">
      <div style="display:flex;align-items:center;gap:10px">
        ...
        <a href="/" style="font-family:var(--font-mono);font-size:14px;font-weight:700;color:var(--green);text-decoration:none">echologs</a>
        <span style="color:var(--border);font-size:14px">/</span>
        <span style="font-family:var(--font-mono);font-size:14px;color:var(--muted)">status</span>
      </div>
      <button
        onclick={toggleTheme}
    ...
    <div style="margin-bottom:40px">
      <h1 style="font-family:var(--font-sans);font-size:1.8rem;font-weight:800;margin:0 0 10px;letter-spacing:-0.5px">{script.name}</h1>
      <div style="display:flex;align-items:center;gap:10px">
        {#if stats.uptime === null}
          <span style="font-family:var(--font-mono);font-size:12px;color:var(--muted)">No data yet</span>
        {:else if stats.uptime >= 95}
          <div style="display:flex;align-items:center;gap:6px;background:var(--green-dim);border:1px solid var(--green-mid);border-radius:20px;padding:4px 14px">
            ...
            <span style="font-family:var(--font-mono);font-size:12px;font-weight:700;color:var(--green)">Operational</span>
          </div>
        {:else if stats.uptime >= 80}
          <div style="display:flex;align-items:center;gap:6px;background:#d9770618;border:1px solid #d9770640;border-radius:20px;padding:4px 14px">
            ...
            <span style="font-family:var(--font-mono);font-size:12px;font-weight:700;color:#d97706">Degraded</span>
          </div>
        {:else}
          <div style="display:flex;align-items:center;gap:6px;background:var(--red-dim);border:1px solid #dc262640;border-radius:20px;padding:4px 14px">
            ...
            <span style="font-family:var(--font-mono);font-size:12px;font-weight:700;color:var(--red)">Disrupted</span>
          </div>
        {/if}
        <span style="font-family:var(--font-mono);font-size:12px;color:var(--muted)">last 60 days</span>
      </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:32px">
      {#each [
        ...
        <div class="dash-card" style="padding:16px 20px">
          <div style="font-family:var(--font-mono);font-size:9px;text-transform:uppercase;letter-spacing:1.5px;color:var(--muted);margin-bottom:8px">{stat.label}</div>
          <div style="font-family:var(--font-mono);font-size:20px;font-weight:700;color:{stat.color}">{stat.value}</div>
        </div>
      {/each}
    </div>
    ...
    <div class="dash-card" style="padding:24px;margin-bottom:24px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
        <div style="font-family:var(--font-mono);font-size:11px;font-weight:700">Last 60 days</div>
        <div style="display:flex;align-items:center;gap:12px;font-family:var(--font-mono);font-size:10px;color:var(--muted)">
          <span style="display:flex;align-items:center;gap:4px">
            <span style="width:8px;height:8px;border-radius:2px;background:var(--green);display:inline-block"></span>
            All passed
          </span>
          <span style="display:flex;align-items:center;gap:4px">
            <span style="width:8px;height:8px;border-radius:2px;background:#d97706;display:inline-block"></span>
            Some failed
          </span>
          <span style="display:flex;align-items:center;gap:4px">
          ...
          <span style="display:flex;align-items:center;gap:4px">
            <span style="width:8px;height:8px;border-radius:2px;background:var(--border);display:inline-block"></span>
            No runs
          </span>
        </div>
      </div>
    ...
    {#if recent.length > 0}
      <div class="dash-card" style="overflow:hidden;margin-bottom:32px">
        <div style="padding:16px 20px;border-bottom:1px solid var(--border)">
          <div style="font-family:var(--font-mono);font-size:11px;font-weight:700">Recent runs</div>
        </div>
        {#each recent as run, i (i)}
          <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 20px;border-bottom:1px solid {i < recent.length - 1 ? 'var(--border)' : 'transparent'}">
            <div style="display:flex;align-items:center;gap:10px">
              <div style="width:7px;height:7px;border-radius:50%;background:{run.status === 'pass' ? 'var(--green)' : 'var(--red)'}"></div>
              <span style="font-family:var(--font-mono);font-size:12px;font-weight:600;color:{run.status === 'pass' ? 'var(--green)' : 'var(--red)'}">{run.status}</span>
            </div>
            <div style="font-family:var(--font-mono);font-size:11px;color:var(--muted)">{formatDuration(run.duration_ms)}</div>
            <div style="font-family:var(--font-mono);font-size:11px;color:var(--muted)">{formatRelative(run.started_at)}</div>
          </div>
        {/each}
      </div>
    {/if}
    ...
    <!-- Footer -->
    <div style="text-align:center;font-family:var(--font-mono);font-size:11px;color:var(--muted)">
      Powered by <a href="/" style="color:var(--green);text-decoration:none">EchoLogs</a>
    </div>
