    ...
    {#each scripts as s (s.id)}
      <button
        ...
        style="
          ...
          border:1px solid {s.streak >= 3 ? '#ff000044' : 'var(--border)'};
          border-radius:12px;
          padding:16px 20px;
          text-align:left;
          cursor:pointer;
      ...
      >
        ...
        <div>
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:3px">
            ...
            {#if s.streak >= 3}
              <span style="font-family:var(--font-mono);font-size:9px;font-weight:700;color:#ff0000;background:#ff000018;border:1px solid #ff000040;padding:2px 7px;border-radius:20px">
                ⚠ {s.streak}x failing
              </span>
            {:else if s.streak >= 1 && s.streak < 3}
              <span style="font-family:var(--font-mono);font-size:9px;font-weight:700;color:var(--red);background:var(--red-dim);border:1px solid #dc262630;padding:2px 7px;border-radius:20px">
                failing
              </span>
            {/if}
          </div>
          <div style="font-family:var(--font-mono);font-size:10px;color:var(--muted)">
            {s.total.toLocaleString()} total run{s.total !== 1 ? 's' : ''}
            {#if s.latestRun}
              · last {formatRelative(s.latestRun.started_at)}
            {/if}
          </div>
        </div>
        ...
        <div>
          <div style="font-family:var(--font-mono);font-size:9px;text-transform:uppercase;letter-spacing:1px;color:var(--muted2);margin-bottom:6px">Last {s.sparkline.length} runs</div>
          <div style="display:flex;align-items:center;gap:3px">
            {#each s.sparkline as run, i (i)}
              <div style="
                width:8px;height:8px;border-radius:2px;
                background:{run.status === 'pass' ? 'var(--green)' : 'var(--red)'};
                opacity:0.85;
        ...
        <!-- Arrow -->
        <div style="font-family:var(--font-mono);font-size:14px;color:var(--muted);text-align:right">→</div>
